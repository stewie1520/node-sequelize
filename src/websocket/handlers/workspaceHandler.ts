...
export class WorkspaceHandler {
  ...
  public setupHandlers(socket: AuthenticatedSocket) {
    ...
    socket.on('leave_workspace', (data) => this.handleLeaveWorkspace(socket, data));
    socket.on('workspace_update', (data) => this.handleWorkspaceUpdate(socket, data));
    socket.on('workspace_activity', (data) => this.handleWorkspaceActivity(socket, data));
  }

  private async handleJoinWorkspace(socket: AuthenticatedSocket, data: { workspaceId: string }) {
    try {
      ...
      const workspace = await Workspace.findOne({
        where: { 
          id: workspaceId,
          authorId: userId // You might need to adjust this based on your workspace access logic
        }
      });
  ...
  private async handleLeaveWorkspace(socket: AuthenticatedSocket, data: { workspaceId: string }) {
    try {
      const { workspaceId } = data;
      const { userId, user } = socket;
  ...
  private async handleWorkspaceUpdate(socket: AuthenticatedSocket, data: { workspaceId: string, updateType: string, updateData: unknown }) {
    try {
      const { workspaceId, updateType, updateData } = data;
      const { userId, user } = socket;

      if (!workspaceId || !updateType) {
        socket.emit('error', { message: 'Workspace ID and update type are required' });
        return;
      }
      ...
      if (!workspace) {
        socket.emit('error', { message: 'Workspace not found or permission denied' });
        return;
      }
      ...
      logger.info(`Workspace ${workspaceId} updated by user ${userId}: ${updateType}`);
    } catch (error) {
      logger.error(`Error updating workspace for user ${socket.userId}:`, error);
      socket.emit('error', { message: 'Failed to update workspace' });
    }
  }

  private handleWorkspaceActivity(socket: AuthenticatedSocket, data: { workspaceId: string, activity: { type: string, details: unknown } }) {
    try {
      const { workspaceId, activity } = data;
      const { userId, user } = socket;

      if (!workspaceId || !activity) {
        socket.emit('error', { message: 'Workspace ID and activity are required' });
        return;
      }
      ...
      logger.debug(`Workspace activity from user ${userId} in workspace ${workspaceId}: ${activity.type}`);
    } catch (error) {
      logger.error(`Error handling workspace activity for user ${socket.userId}:`, error);
