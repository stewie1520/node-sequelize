...
export class NotificationHandler {
  ...
  private async handleNotificationRead(
    ...
    try {
      ...
      logger.error(
        `Error marking notification as read for user ${socket.userId}:`,
        error,
      );
  ...
  private async handleNotificationPreferences(
    ...
    try {
      ...
      logger.error(
        `Error updating notification preferences for user ${socket.userId}:`,
        error,
      );
      socket.emit("error", {
  ...
  public async sendWorkspaceNotification(
    workspaceId: string,
    notification: {
      id?: string;
      type: string;
