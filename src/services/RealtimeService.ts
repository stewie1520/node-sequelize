...
export class RealtimeService {
  ...
  public async notifyUserCreated(userId: string, userData: UserData): Promise<void> {
    try {
  ...
  public async notifyUserUpdated(userId: string, userData: UserData): Promise<void> {
    try {
      await this.socketService.emitToUser(userId, 'profile_updated', {
        type: 'user_updated',
        data: userData,
  ...
  public async notifyWorkspaceCreated(workspaceId: string, creatorId: string, workspaceData: WorkspaceData): Promise<void> {
    try {
  ...
  public async notifyWorkspaceUpdated(workspaceId: string, updatedBy: string, updateData: WorkspaceData): Promise<void> {
    try {
      await this.socketService.emitToWorkspace(workspaceId, 'workspace_updated', {
        updatedBy,
        data: updateData,
        timestamp: new Date().toISOString()
      });
      ...
      if (this.notificationHandler) {
        await this.notificationHandler.sendWorkspaceNotification(workspaceId, {
      ...
      logger.debug(`Workspace update notification sent to workspace ${workspaceId}`);
    } catch (error) {
      logger.error('Error sending workspace update notification:', error);
    }
  }

  public async notifyWorkspaceDeleted(workspaceId: string, deletedBy: string): Promise<void> {
    try {
      await this.socketService.emitToWorkspace(workspaceId, 'workspace_deleted', {
        workspaceId,
        deletedBy,
        timestamp: new Date().toISOString()
      });

      logger.debug(`Workspace deletion notification sent to workspace ${workspaceId}`);
    } catch (error) {
  ...
  // Activity tracking
  public async trackUserActivity(userId: string, workspaceId: string, activity: ActivityData): Promise<void> {
    try {
      const activityData = {
        userId,
        workspaceId,
      ...
      await this.socketService.emitToWorkspace(workspaceId, 'user_activity', activityData);

      // Store activity in Redis for analytics (optional)
      await this.pubsub.publish('activity_log', JSON.stringify(activityData));

      logger.debug(`Activity tracked for user ${userId} in workspace ${workspaceId}`);
    } catch (error) {
  ...
  public async sendSystemNotification(userId: string, notification: NotificationData): Promise<void> {
    try {
      if (this.notificationHandler) {
